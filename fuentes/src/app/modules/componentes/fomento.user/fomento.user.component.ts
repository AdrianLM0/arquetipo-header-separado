...
/* 	actualizarLista() {
	...
	generarListaPrivilegios() {
		const res:any = [];
		this.listaPrivilegios.forEach((x:any) => res.push(x.nombre));
		return res;
	}

	clickBotonCambiaRol(event:any) {
	...
	guardarPrivilegioEnLocalStorage() {
		if (this.privilegioUsuario) {
			let aux = this.listaPrivilegios.find(
				(x:any) => x.nombre === this.privilegioUsuario,
			).codigo;
